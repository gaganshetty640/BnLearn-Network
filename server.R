...
server<-(function(input, output, session){
  output$network <- renderVisNetwork({
    ...
    nodes <- nodes(fittedmodel)
    edges <- as.data.frame(arcs(fittedmodel))
    label <- nodes
    ...
    Network <- visNetwork(nodes_tab,edges_tab,main = "CUSTOMER MAP")%>%
      visPhysics(enabled = F)%>%
      visInteraction(navigationButtons=T,
                     selectConnectedEdges=F,
                     multiselect = T)%>%
      visOptions(autoResize = T,highlightNearest = list(enabled = T, degree = 2, hover = T))%>%
  ...
  observeEvent(input$Initialnode, {
    ...
    output$prob_values <- renderUI({
      ...
      num_inputs <- lapply(1:nrow(cc), function(i){
        numericInput(inputId = paste0(input$Initialnode,i), label = paste0("State : ",as.character(cc[input$Initialnode][i,1]), " %"), value = (round(cc$Freq[i]*100,4)), min = 0, max = 100)
    ...
    # shinyjs::hidden(num_inputs)
     ...
     observe({
          ...
          for (i in 1:nrow(cc)){
           shinyjs::show(id= paste0(input$Initialnode,i))
          }
  ...
  observeEvent(input$simulate,{
    ...
    combined_query$BeforeEvidence <- round(combined_query$BeforeEvidence,5)
    combined_query$AfterEvidence <- round(combined_query$AfterEvidence,5)
    
    
    # combined_query_f <- simulate_f()
               output$queryresults <- renderDataTable(combined_query)
