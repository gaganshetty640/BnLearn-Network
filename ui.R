...
# UI ####
ui <- fluidPage(
  fluidRow(column(div(img(src="C5.jpeg", width = 300, height =70, style="text-align: center;")),width=2),
           h1("BAYESIAN SIMULATION"),align = "center"),
  br(),
  hr(),
  br(),
  fluidRow(),
  ...
  fluidPage(
    ...
    fluidRow(column(10,
            visNetworkOutput("network",height = "700",width = "80%")
        )),
    fluidRow(h1("Simulation"),align = "center"),
    br(),br(),br(),
    fluidRow(
            column(width = 4,align='left',
                   selectInput("Initialnode", "Select Node :",choices = (data.frame(nodes(fittedmodel))), selected = "LUNCH_BAG_APPLE_DESIGN" ),
                   br(),
                  ),
            column(width = 2,align = "center",
                   uiOutput("prob_values")
                   ),
                   #,height=85,style="width:84px; font-size:11px;padding: 10px 2px; margin:0%")
            column(width = 4,align='left',
                   selectInput("Targetnode", "Target Node:",choices = (data.frame(nodes(fittedmodel))), selected = "CLTV_12_months" ),
                   br(),
                  ),
          br(),br(),br(),br(),
          #column(width =1),
            column(width = 2,align = "left",#offset = 0,style = "background-color:rgba(255,255,255,0.8);border: 1px solid black;",
                   
                   actionButton('simulate', 'Simulate',class = "btn btn-primary")),
              ),
